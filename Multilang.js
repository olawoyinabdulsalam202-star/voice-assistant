...
(function () {
  ...
  function detectLanguage(text) {
    ...
    // 1. Script-based rules first (most reliable)
    ...
    for (const rule of LANG_RULES) {
      ...
      const hits = tokens.filter(t => rule.words.includes(t)).length;
  ...
  function patchAskKairos() {
    if (!window.askKairos) {
      // retry once in case of load-order race
      setTimeout(patchAskKairos, 500);
      return;
    }
    if (window.askKairos._langPatched) return;

    const original = window.askKairos;

    window.askKairos = async function (userMessage, ...rest) {
      const instruction = window.kairosLang.getInstruction(userMessage);
      const augmented   = instruction ? `${instruction}\n\n${userMessage}` : userMessage;
      return original(augmented, ...rest);
    };

    window.askKairos._langPatched = true;
  }
